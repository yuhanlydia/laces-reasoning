#!/bin/bash
# Plan B 0.4B: s0 (VAE) → s1 (alpha) → s2 (DiT) → sample
# Single-GPU, all stages sequential on same card
set -e
...
echo "========================================="
echo " Plan B 0.4B: s0 → s1 → s2 → sample"
echo " Config : $CONFIG"
echo " Name   : $NAME"
echo " GPU    : $GPU"
echo "========================================="
...
echo "[$(date '+%H:%M:%S')] 0.4B pipeline done!"
