#!/usr/bin/env python3
"""Re-preprocess WikiText-2 with sliding window tokenization.

Creates overlapping 512-token chunks with stride 256, maximizing data usage.
Output: preprocessed_data/wikitext2_full/tokens/{train,val}/
"""
import os
import json
import numpy as np
...
SEQ_LEN = 512
STRIDE = 256
BASE_DIR = Path(os.environ.get("DIFFRWKV_DATA_DIR", "preprocessed_data")) / "wikitext2_full"
MODEL_NAME = "RWKV/RWKV7-Goose-World3-1.5B-HF"

def process_split(split_name, out_dir):
    out_dir.mkdir(parents=True, exist_ok=True)
    ds = load_dataset("wikitext", "wikitext-2-raw-v1", split=split_name)
