"""Tokenize OpenWebText with RWKV tokenizer + Qwen 768-dim latents."""
import json, os
import numpy as np
...
qw_tokenizer = AutoTokenizer.from_pretrained(QWEN, trust_remote_code=True)
...
with open(OWT_JSONL) as f:
    with tqdm(total=total, desc="Encoding") as pbar:
        while True:
            ...
            enc = tok(batch, truncation=True, max_length=MAX_LENGTH, padding="max_length", return_tensors="np")
            for j in range(len(batch)):
                np.savez(os.path.join(OUT_TOKENS, f"{idx + j:08d}_tokens.npz"),
                         input_ids=enc["input_ids"][j].astype(np.int32),
                         attention_mask=enc["attention_mask"][j].astype(bool))

            qw_enc = qw_tokenizer(batch, truncation=True, max_length=MAX_LENGTH,
            ...
            for j in range(len(batch)):
                np.save(os.path.join(OUT_LATENTS, f"{idx + j:08d}.npy"), emb[j])

            idx += len(batch)
            pbar.update(len(batch))

print(f"Done → {OUT_TOKENS}, {OUT_LATENTS}")
print("Update MAX_SAMPLES to process all 8M")
