...
class DirectLatentAverageMethod:
    """Direct latent average: mean of per-agent latent z, no PCA, no resample."""
    ...
    def run(self, task: TaskSpec) -> MethodResult:
        ...
        def process():
            ...
            z_avg = torch.stack(all_z, dim=0).mean(dim=0).to(self.dtype)
            plan_states = [self.model.predict_states(z_avg) for _ in range(self.H)]
        ...
        return MethodResult(
            method_name="direct_latent_avg", answer=answer.strip(),
            is_correct=is_correct, time_ms=time_ms, payload_bytes=z_bytes,
            metadata={"pca_k": 0, "resample": False, "n_agents": len(task.agents)},
        )
