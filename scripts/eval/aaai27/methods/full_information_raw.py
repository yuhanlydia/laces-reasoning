...
class FullInformationRawMethod:
    """Full-information raw: one agent sees ALL information concatenated."""
    ...
    def run(self, task: TaskSpec) -> MethodResult:
        def process():
            full_text = "\n\n".join([a.text for a in task.agents])
            prompt = f"{full_text}\n\n{task.query}\nAnswer:"
        ...
        payload_bytes = len("\n\n".join([a.text for a in task.agents]).encode("utf-8"))

        return MethodResult(
            method_name="full_information_raw", answer=answer.strip(),
