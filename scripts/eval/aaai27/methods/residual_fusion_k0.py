...
class ResidualFusionK0Method:
    """Residual fusion k=0: subtract mean, fuse residuals, resample (no PCA removal)."""
    ...
    def run(self, task: TaskSpec) -> MethodResult:
        ...
        def process():
            ...
            z_residuals = z_stack - z_mean

            fused_resid = z_residuals.mean(dim=0, keepdim=True)
            cond = (z_mean + fused_resid).to(self.dtype)
        ...
        return MethodResult(
            method_name="residual_fusion_k0", answer=answer.strip(),
            is_correct=is_correct, time_ms=time_ms, payload_bytes=z_bytes,
            metadata={"pca_k": 0, "resample": True, "n_agents": len(task.agents)},
        )
