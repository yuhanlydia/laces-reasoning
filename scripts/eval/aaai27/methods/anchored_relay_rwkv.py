...
class AnchoredRelayRWKVMethod:
    """Anchored relay: encode each agent → latent → anchor blend → resample → decode."""
    ...
    def run(self, task: TaskSpec) -> MethodResult:
        ...
        def process():
            ...
            for i in range(1, len(all_z)):
                ...
                z_next = self.sample_trajectory_cfg(
                    self.model, cond.unsqueeze(0), steps=100, cfg_scale=3.0,
                    device=self.device, dtype=self.dtype,
                )
                z_cur = z_next[0].float().mean(dim=0)
        ...
        return MethodResult(
            method_name="anchored_relay", answer=answer.strip(),
