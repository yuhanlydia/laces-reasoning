...
class RenormalizedRelayRWKVMethod:
    """Renormalized relay: z_{t+1} = sample(z_t), then pull norm back to target."""
    ...
    def run(self, task: TaskSpec) -> MethodResult:
        ...
        def process():
            ...
            for i in range(1, len(all_z)):
                ...
                z_next = Z[0].float().mean(dim=0)
                cur_norm = z_next.norm()
                z_cur = z_next * (self.target_norm / cur_norm.clamp(min=1e-6))
        ...
        return MethodResult(
            method_name="renormalized_relay", answer=answer.strip(),
            is_correct=is_correct, time_ms=time_ms, payload_bytes=z_bytes,
            metadata={"anchor": 0.0, "renorm": self.target_norm, "n_agents": len(task.agents)},
        )
