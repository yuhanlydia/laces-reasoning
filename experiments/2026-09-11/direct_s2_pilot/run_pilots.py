...
root=Path('/root/laces-reasoning'); out=root/'outputs_eval/direct_s2_review_2026-09-11'
base=dict(os.environ,PYTHON=str(root/'.venv/bin/python'),OMP_NUM_THREADS='4',TOKENIZERS_PARALLELISM='false',GPU='0',MODE='train',NUM_STEPS='2',GROUP_SIZE='4',LR='1e-6',DIFFUSION_STEPS='32',NATIVE_CONTROL_STEPS='1000',EVAL_EVERY='2',SAVE_EVERY='2',DEV_LIMIT='2',DATA_DIR=str(root/'data/mmlu_pro_pilot'),CKPT_DIR=str(root/'outputs_dynamic_basis/laces-2.9B-dynlowrank-r32-s0frozen-joint-b4-50k-fla03/step_00030000'))
runs={'distill':{'OBJECTIVE':'distill','OUTPUT_DIR':'results/mmlu_pro/pilot_distill_s42'},'grpo_direct':{'OBJECTIVE':'grpo','OUTPUT_DIR':'results/mmlu_pro/pilot_grpo_direct_s42'},'distill_grpo':{'OBJECTIVE':'grpo','OUTPUT_DIR':'results/mmlu_pro/pilot_distill_grpo_s42','INIT_S2':'results/mmlu_pro/pilot_distill_s42/best_dev.pt'}}
...
with (out/'gpu_utilization.csv').open('w') as monitor:
 ...
 while active:
  ...
  (out/'pilot_status.json').write_text(json.dumps({'runs':done,'active':list(active),'peak_gpu_memory_mib':peak},indent=2)+'\n')
