...
S2_NAME="${BASE_NAME}-prefix-suffix-blend${STATE_BLEND_LABEL}-s2-rwkv-s1-birwkv-ddpm"
S1_CKPT="${S1_CKPT:-outputs_relay/${S1_NAME}/step_$(printf '%08d' "${S1_STEPS}")}"

print_header "${CONTEXT_LENGTH} ${BACKBONE} prefix/suffix mixed trajectory S1=RWKV S2=BiRWKV DDPM basis=${BASIS} blend=${STATE_BLEND}"
